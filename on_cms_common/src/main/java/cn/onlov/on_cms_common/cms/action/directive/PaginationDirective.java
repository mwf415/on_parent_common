...
import static cn.onlov.on_cms_common.core.web.util.FrontUtils.PARAM_SYS_PAGE;
import static cn.onlov.on_cms_common.core.web.util.FrontUtils.PARAM_USER_PAGE;
import static cn.onlov.on_cms_common.core.web.util.FrontUtils.getTplPath;
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		if ("1".equals(content)) {
			...
			if (!StringUtils.isBlank(sysPage)) {
				String tpl = Constants.TPL_STYLE_PAGE_CONTENT + sysPage + Constants.TPL_SUFFIX;
				env.include(tpl, cn.onlov.on_cms_common.common.web.Constants.UTF8, true);
			} else if (!StringUtils.isBlank(userPage)) {
				String tpl = getTplPath(site.getSolutionPath(),
						Constants.TPLDIR_STYLE_PAGE, userPage);
				env.include(tpl, cn.onlov.on_cms_common.common.web.Constants.UTF8, true);
			} else {
				// 没有包含分页
			}
