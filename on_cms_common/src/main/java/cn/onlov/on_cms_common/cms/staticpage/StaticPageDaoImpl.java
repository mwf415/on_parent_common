...
import cn.onlov.on_cms_common.common.hibernate4.HibernateSimpleDao;
...
public class StaticPageDaoImpl extends HibernateSimpleDao implements
	...
	public int channelStatic(HttpServletRequest request, HttpServletResponse response,
		...
		if(request!=null&&response!=null){
			...
			sessionProvider.setAttribute(request, response, 
					cn.onlov.on_cms_common.cms.api.Constants.STATIC_TOTAL_COUNT_KEY, totalCount);
		}
		...
		while (channels.next()) {
			...
			if(site.getPageSync()&&es==null){
				es=Executors.newFixedThreadPool(cn.onlov.on_cms_common.core.Constants.DISTRIBUTE_THREAD_COUNT);
			}
			...
			if(request!=null&&response!=null){
				sessionProvider.setAttribute(request, response, 
						cn.onlov.on_cms_common.cms.api.Constants.STATIC_CURR_COUNT_KEY, count);
			}
		}
		if(es!=null){
			es.shutdown();
	...
	public void channelStatic(Channel c, boolean firstOnly, Configuration conf,
		...
		totalPage=totalPage>10? cn.onlov.on_cms_common.cms.api.Constants.MAX_STATIC_COUNT:totalPage;
		for (int i = 1; i <= totalPage; i++) {
			filename = c.getStaticFilename(i);
			createChannelPage(es,site,conf, data, c, filename, i,false);
			//手机静态页页面
