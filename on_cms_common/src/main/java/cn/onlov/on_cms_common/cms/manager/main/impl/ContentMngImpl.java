...
import cn.onlov.on_cms_common.cms.manager.main.ChannelCountMng;
...
import cn.onlov.on_cms_common.cms.manager.main.ContentTxtMng;
import cn.onlov.on_cms_common.cms.manager.main.ContentTypeMng;
import cn.onlov.on_cms_common.cms.service.ChannelDeleteChecker;
import cn.onlov.on_cms_common.cms.service.ContentListener;
import cn.onlov.on_cms_common.cms.staticpage.StaticPageSvc;
import cn.onlov.on_cms_common.cms.staticpage.exception.ContentNotCheckedException;
import cn.onlov.on_cms_common.cms.staticpage.exception.GeneratedZeroStaticPageException;
import cn.onlov.on_cms_common.cms.staticpage.exception.StaticPageNotOpenException;
import cn.onlov.on_cms_common.cms.staticpage.exception.TemplateNotFoundException;
import cn.onlov.on_cms_common.cms.staticpage.exception.TemplateParseException;
...
public class ContentMngImpl implements ContentMng, ChannelDeleteChecker {
	...
	public Pagination getPageBySite(String title, Integer typeId, Integer currUserId, Integer inputUserId, boolean topLevel,
                                    boolean recommend, Content.ContentStatus status, Integer siteId, int orderBy, int pageNo, int pageSize){
	...
	public Pagination getPageForMember(String title, Integer channelId,Integer siteId,Integer modelId, Integer memberId, int pageNo, int pageSize) {
		return dao.getPageForMember(Content.CONTENT_QUERY_NOT_SHARE,
				title, null,memberId,memberId, false, false, Content.ContentStatus.all, null, siteId,modelId,  channelId, 0, pageNo,pageSize);
	}
	...
	public List<Content> getListForMember(String title, Integer channelId,
		...
		return dao.getList(title, null,memberId,memberId, false,
				false, Content.ContentStatus.all, null, siteId,modelId,
	...
	public Content save(Content bean, ContentExt ext, ContentTxt txt, Integer[] channelIds,
		...
		//文章操作记录
		contentRecordMng.record(bean, user, ContentRecord.ContentOperateType.add);
		//栏目内容发布数（未审核通过的也算）
	...
	public Content update(Content bean, ContentExt ext, ContentTxt txt,String[] tagArr,
		...
		if (draft != null) {
			if (draft) {
				...
				if (bean.getStatus() == ContentCheck.DRAFT) {
		...
		contentRecordMng.record(bean, user, ContentRecord.ContentOperateType.edit);
		//打赏固定值
	...
	public Content update(CmsUser user, Content bean, ContentRecord.ContentOperateType operate){
		// 保存操作记录
	...
	public Content check(Integer id, CmsUser user) {
		...
		if (userStep < contentStep) {
			content.setCheckResult(Content.CheckResultStatus.nopass);
			return content;
		}
		...
		contentRecordMng.record(content, user, ContentRecord.ContentOperateType.check);
	...
	public Content reject(Integer id, CmsUser user,  Byte step, String opinion) {
		...
		contentRecordMng.record(content, user, ContentRecord.ContentOperateType.rejected);
		// 执行监听器
		//afterChange(content, mapList);
		return content;
	}
	...
	public Content cycle(CmsUser user,Integer id) {
		...
		// 执行监听器
		//afterChange(content, mapList);
		contentRecordMng.record(content, user, ContentRecord.ContentOperateType.cycle);
		return content;
	}
