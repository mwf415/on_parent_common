...
import cn.onlov.on_cms_common.common.hibernate4.HibernateTree;
import cn.onlov.on_cms_common.common.hibernate4.PriorityComparator;
...
import cn.onlov.on_cms_common.cms.entity.main.base.BaseChannel;
...
public class Channel extends BaseChannel implements HibernateTree<Integer>, PriorityInterface, Cloneable {
	...
	public String getUrlStatic(Boolean whole, int pageNo) {
		...
		if (!StringUtils.isBlank(filename)) {
			...
			// 默认静态页面访问路径
			url.append(Constants.SPT).append(getPath());
			if (pageNo > 1) {
				url.append("_").append(pageNo);
				url.append(site.getStaticSuffix());
			} else {
				if (getHasContent()) {
					url.append(Constants.SPT);
				} else {
					url.append(site.getStaticSuffix());
				}
			}
	...
	public String getStaticFilename(int pageNo) {
		...
		if (!StringUtils.isBlank(filename)) {
			...
			if (getHasContent()) {
				url.append(Constants.SPT).append(Constants.INDEX);
				if (pageNo > 1) {
					url.append("_").append(pageNo);
				}
				url.append(suffix);
