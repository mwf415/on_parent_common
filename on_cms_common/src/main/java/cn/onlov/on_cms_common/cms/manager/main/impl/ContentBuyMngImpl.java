...
import cn.onlov.on_cms_common.cms.dao.main.ContentBuyDao;
	...
	private void initAliPayUrl(){
		if(getAliPayUrl()==null){
			setAliPayUrl(PropertyUtils.getPropertyValue(
					new File(realPathResolver.get(Constants.JEECMS_CONFIG)),ALI_PAY_URL));
		}
	}
	...
	private void initWeiXinPayUrl(){
		if(getWeiXinPayUrl()==null){
			setWeiXinPayUrl(PropertyUtils.getPropertyValue(
					new File(realPathResolver.get(Constants.JEECMS_CONFIG)),WEIXIN_ORDER_QUERY_URL));
		}
	}
