...
import cn.onlov.on_cms_common.common.email.EmailSendTool;
...
import cn.onlov.on_cms_common.core.dao.UnifiedUserDao;
...
public class UnifiedUserMngImpl implements UnifiedUserMng {
	...
	private void senderEmail(final String username, final String to,
		...
		text = StringUtils.replace(text, "${username0}", username);
		text = StringUtils.replace(text, "${username}", encodeUsername);
