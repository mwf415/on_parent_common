...
import cn.onlov.on_cms_common.core.dao.CmsSiteDao;
...
import cn.onlov.on_cms_common.core.manager.CmsUserSiteMng;
...
public class CmsSiteMngImpl implements CmsSiteMng {
	...
	public List<Map<String, Object>> getAttrListBySiteId(Integer siteId) {
		List<Map<String, Object>> list = dao.getAttrListBySiteId(siteId);
		return list;
	}

	@Override
	public Integer deleteAttrListBySiteId(Integer[] ids) {
		Integer count=0;
		for (int i = 0, len = ids.length; i < len; i++) {
			count += dao.deleteAttrListBySiteId(ids[i]);
		}
		return count;
	}
}