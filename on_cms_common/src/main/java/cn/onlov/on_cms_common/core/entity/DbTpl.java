...
import cn.onlov.on_cms_common.core.entity.base.BaseDbTpl;
...
public class DbTpl extends BaseDbTpl implements Tpl {
	...
	public static String[] getParentDir(String path) {
		Assert.notNull(path);
		if (!path.startsWith(Constants.SPT)) {
			throw new IllegalArgumentException("path must start with /");
		}
		List<String> list = new ArrayList<String>();
		int index = path.indexOf(Constants.SPT, 1);
		while (index >= 0) {
			list.add(path.substring(0, index));
			index = path.indexOf(Constants.SPT, index + 1);
		}
