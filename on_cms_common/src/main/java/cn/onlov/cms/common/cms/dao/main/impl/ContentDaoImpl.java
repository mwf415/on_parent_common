...
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.all;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.checked;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.draft;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.passed;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.prepared;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.recycle;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.rejected;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.contribute;
import static cn.onlov.cms.common.cms.entity.main.Content.ContentStatus.pigeonhole;
		...
		if(attr!=null&&!attr.isEmpty()){
			...
			while(keyIterator.hasNext()){
				...
				if(StringUtils.isNotBlank(key)&&StringUtils.isNotBlank(value)){
					if(operate.equals(AbstractContentDirective.PARAM_ATTR_EQ)){
						f.append(" and bean.attr[:k"+key+"]=:v"+key).setParam("k"+key, key).setParam("v"+key, value);
					}else if(operate.equals(AbstractContentDirective.PARAM_ATTR_START)){
						f.append(" and bean.attr[:k"+key+"] like :v"+key).setParam("k"+key, key).setParam("v"+key, value+"%");
					}else if(operate.equals(AbstractContentDirective.PARAM_ATTR_END)){
						f.append(" and bean.attr[:k"+key+"] like :v"+key).setParam("k"+key, key).setParam("v"+key, "%"+value);
					}else if(operate.equals(AbstractContentDirective.PARAM_ATTR_LIKE)){
						f.append(" and bean.attr[:k"+key+"] like :v"+key).setParam("k"+key, key).setParam("v"+key, "%"+value+"%");
					}else if(operate.equals(AbstractContentDirective.PARAM_ATTR_IN)){
						...
						if(operate.equals(AbstractContentDirective.PARAM_ATTR_GT)){
							if(floatValue>=0){
					 	...
					 	}else if(operate.equals(AbstractContentDirective.PARAM_ATTR_GTE)){
					 		if(floatValue>=0){
						...
						}else if(operate.equals(AbstractContentDirective.PARAM_ATTR_LT)){
							if(floatValue>=0){
								f.append(" and ((abs(bean.attr[:k"+key+"])<:v"+key+" and bean.attr[:k"+key+"]>=0) or bean.attr[:k"+key+"]<=0)").setParam("k"+key, key).setParam("v"+key, floatValue);
							}else{
								f.append(" and ((abs(bean.attr[:k"+key+"])>:v"+key+" and bean.attr[:k"+key+"]<0) or bean.attr[:k"+key+"]>=0)").setParam("k"+key, key).setParam("v"+key, -floatValue);
							}
						}else if(operate.equals(AbstractContentDirective.PARAM_ATTR_LTE)){
							if(floatValue>=0){
								f.append(" and ((abs(bean.attr[:k"+key+"])<=:v"+key+" and bean.attr[:k"+key+"]>=0) or bean.attr[:k"+key+"]<=0)").setParam("k"+key, key).setParam("v"+key, floatValue);
							}else{
								f.append(" and ((abs(bean.attr[:k"+key+"])>=:v"+key+" and bean.attr[:k"+key+"]<0) or bean.attr[:k"+key+"]>=0)").setParam("k"+key, key).setParam("v"+key, -floatValue);
