package cn.onlov.cms.common.common.util;
...
import cn.onlov.cms.common.common.web.ClientCustomSSL;
					...
					if (returnCode.equalsIgnoreCase("FAIL")) {
						...
						if (map.get("err_code") != null) {
							...
							return FrontUtils.getTplPath(request, site.getSolutionPath(),
									Constants.TPLDIR_SPECIAL,CONTENT_CODE_WEIXIN);
						}
					}
				}
			...
			if (StringUtils.isNotBlank(config.getTransferApiPassword())) {
				...
				if (returnCode.equalsIgnoreCase("FAIL")) {
					...
					if (map.get("err_code") != null) {
						...
						model.addAttribute("content", content);
						return FrontUtils.getTplPath(request, site.getSolutionPath(),
								Constants.TPLDIR_SPECIAL,CONTENT_PREPAY);
					}
				}
				return FrontUtils.showMessage(request, model, "error.connect.timeout");
			} else {
	...
	public  static void noticeWeChatSuccess(String weiXinPayUrl){
		...
		try{    		
			if(StringUtils.isNotBlank(weiXinPayUrl)){
				String s=HttpClientUtil.post(weiXinPayUrl, xmlWeChat, cn.onlov.cms.common.common.web.Constants.UTF8);
			}
		}catch(Exception e){
			e.printStackTrace();
		}
